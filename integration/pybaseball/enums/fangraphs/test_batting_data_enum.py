...
def test_enums_vs_fangraphs_column_list() -> None:
    ...
    sample_pitching_url = "https://www.fangraphs.com/leaders.aspx?pos=all&stats=bat&lg=all&qual=y&type=8&season=2020&month=0&season1=2020&ind=0"
    ...
    current_leaderboard_items = sorted(
        [str(x).split('.')[1] for x in FangraphsBattingStats.ALL()
         if x not in  [FangraphsBattingStats.COMMON, FangraphsBattingStats.LINE_BREAK]]
    )
